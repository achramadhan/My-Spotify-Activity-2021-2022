...
### Average Listened Songs by Hour
```{r}
streaming_history %>% mutate(hour = hour(end_time)) %>% 
  group_by(hour) %>% 
  summarise(avg_listened = mean(hour)) %>%
  ggplot(aes(factor(hour), avg_listened))+
  geom_bar(stat='identity', fill='orange')+
  geom_text(aes(label=avg_listened),vjust=-0.7, size=3, angle = 30 )+
  xlab("hour")
```
